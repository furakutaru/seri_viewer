...
export async function parseCatalog(catalogUrl: string) {
  try {
    ...
    rows.slice(1).each((index, element) => {
      ...
      // テーブル構造: No, 写真, 動画, ＨＰ, 性, 毛色, 生月日, 父馬名, 母馬名, 地区, 申込者名, 飼養者名, 結果, 購買者名, 落札額
      ...
      const sireName = cleanText($(cells[7]).text());
      const damName = cleanText($(cells[8]).text());
...
export async function parsePdfMeasurements(pdfUrl: string) {
  try {
    ...
    // キャッシュを確認
    let buffer = await getFromCache(pdfUrl, 'pdf');
    
    if (!buffer) {
      console.log(`[Cache] Miss: ${pdfUrl}, fetching from URL...`);
      const response = await fetch(pdfUrl);
      const arrayBuffer = await response.arrayBuffer();
      buffer = Buffer.from(arrayBuffer);
      // キャッシュに保存
      await saveToCache(pdfUrl, 'pdf', buffer);
    } else {
      console.log(`[Cache] Hit: ${pdfUrl}`);
    }
